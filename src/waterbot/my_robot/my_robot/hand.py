...
class CameraWebSocketNode(Node):
    def __init__(self):
        ...
        self.FORWARD_V = 0.05
        self.TURN_V    = 0.07
        self.BACKWARD_V = -0.05

        self.prev_water_state = None
        ...
        try:
            async for message in self.ws:
                try:
                    ...
                    self.get_logger().warning("⚠️ JSON 解碼失敗")
                    continue

                # ✅ 1. 先處理 move_action（新增）
                move = data.get("move_action")
                if move:
                    twist = Twist()
                    if move == "forward":
                        twist.linear.x = self.FORWARD_V
                    elif move == "backward":
                        twist.linear.x = self.BACKWARD_V
                    elif move == "left":
                        twist.linear.x = self.FORWARD_V * 0.4  # 稍微往前
                        twist.angular.z = self.TURN_V
                    elif move == "right":
                        twist.linear.x = self.FORWARD_V * 0.4  # 稍微往前
                        twist.angular.z = -self.TURN_V
                    elif move == "stop":
                        ...
                        self.get_logger().warn(f"⚠️ 未知 move_action: {move}")
                        twist = None

                    if twist:
                        self.vel_pub.publish(twist)
                        self.get_logger().info(f"🚗 發佈速度: move={move}")

                water = data.get("water_action")
                if water == "on" and self.prev_water_state != "on":
                    self.pump_pub.publish(Bool(data=True))
                    self.prev_water_state = "on"
                elif water == "off" and self.prev_water_state != "off":
                    self.pump_pub.publish(Bool(data=False))
                    self.prev_water_state = "off"

                # ✅ 2. 繼續保留你原本的 YOLO 指令處理區塊
                cmd   = data.get("cmd")
